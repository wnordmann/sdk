...
describe('MapConfigTransfrormService', function() {
  ...
  it('transforms MapBox source correctly (two-way)', function() {
    ...
    assert.equal(JSON.stringify(result), JSON.stringify(expected));
    var output = MapConfigTransformService.write(result);
    var expectedFromWrite = {'map':{'sources':{'0':{'ptype':'gxp_mapboxsource'}},'layers':[{'name':'geography-class','title':'Geography Class','visibility':true,'source':'0'}],'center':[0,0],'projection':'EPSG:102113','zoom':3}};
    assert.equal(JSON.stringify(output), JSON.stringify(expectedFromWrite));
  });
  ...
  it('transforms Bing source correctly (two-way)', function() {
    ...
    assert.equal(JSON.stringify(result), JSON.stringify(expected));
    var output = MapConfigTransformService.write(result);
    var expectedFromWrite = {'map':{'sources':{'0':{'ptype':'gxp_bingsource','apiKey':'AkGbxXx6tDWf1swIhPJyoAVp06H0s0gDTYslNWWHZ6RoPqMpB9ld5FY1WutX8UoF'}},'layers':[{'name':'AerialWithLabels','title':'Bing Aerial With Labels','visibility':true,'group':'background','source':'0'},{'name':'Aerial','title':'Bing Aerial','visibility':false,'group':'background','source':'0'},{'name':'Road','title':'Bing Roads','visibility':false,'group':'background','source':'0'}],'center':[0,0],'projection':'EPSG:102113','zoom':3}};
    assert.equal(JSON.stringify(output), JSON.stringify(expectedFromWrite));
  });
  ...
  it('transforms ArcGIS Rest source correctly (two-way)', function() {
    ...
    assert.equal(JSON.stringify(result), JSON.stringify(expected));
    var output = MapConfigTransformService.write(result);
    var expectedFromWrite = {'map':{'sources':{'0':{'url':'http://cga6.cga.harvard.edu/arcgis/rest/services/darmc/roman/MapServer','ptype':'gxp_arcrestsource'}},'layers':[{'name':'Roman and Medieval Civilization','title':'Roman and Medieval Civilization','visibility':true,'layerid':'show:0','source':'0'}],'center':[4263719.1382864,5532513.4852863],'projection':'EPSG:102113','zoom':3}};
    assert.equal(JSON.stringify(output), JSON.stringify(expectedFromWrite));
  });
